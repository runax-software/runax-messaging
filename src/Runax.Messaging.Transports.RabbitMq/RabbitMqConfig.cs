...
/// <summary>
/// Transport config for RabbitMQ. Attach to a bus with
/// <c>bus.AddTransport(new RabbitMqConfig { Uri = ... })</c> (or the delegate /
...
/// </summary>
public sealed class RabbitMqConfig : TransportConfig
{
    /// <inheritdoc />
    public override string SystemName => RabbitMqTransport.TransportName;

    /// <inheritdoc />
    protected override IMessagingTransport CreateTransport(TransportContext context) =>
        new RabbitMqTransport(this, context.Services.GetRequiredService<ILogger<RabbitMqTransport>>());
    ...
    protected override void ConfigureServices(IServiceCollection services, string busName)
    {
        ...
        services.AddHealthChecks().Add(new HealthCheckRegistration(
            $"runax:{busName}",
            sp => new RabbitMqHealthCheck(sp.GetRequiredKeyedService<IMessagingTransport>(busName)),
            failureStatus: null,
            tags: null));
    }
