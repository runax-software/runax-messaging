...
/// <summary>
/// Transport config for Apache Kafka. Attach to a bus with
/// <c>bus.AddTransport(new KafkaConfig { BootstrapServers = ... })</c> (or the delegate /
...
/// </summary>
public sealed class KafkaConfig : TransportConfig
{
    /// <inheritdoc />
    public override string SystemName => KafkaTransport.TransportName;

    /// <inheritdoc />
    protected override IMessagingTransport CreateTransport(TransportContext context) =>
        new KafkaTransport(this, context.Services.GetRequiredService<ILogger<KafkaTransport>>());
    ...
    protected override void ConfigureServices(IServiceCollection services, string busName)
    {
        ...
        services.AddHealthChecks().Add(new HealthCheckRegistration(
            $"runax:{busName}",
            sp => new KafkaHealthCheck(sp.GetRequiredKeyedService<IMessagingTransport>(busName)),
            failureStatus: null,
            tags: null));
    }
