...
/// <summary>
/// Transport config for Amazon SQS. Attach to a bus with
/// <c>bus.AddTransport(new SqsConfig { Region = ... })</c> (or the delegate /
...
/// </summary>
public sealed class SqsConfig : TransportConfig
{
    /// <inheritdoc />
    public override string SystemName => SqsTransport.TransportName;

    /// <inheritdoc />
    protected override IMessagingTransport CreateTransport(TransportContext context) =>
        new SqsTransport(this, context.Services.GetRequiredService<ILogger<SqsTransport>>());
    ...
    protected override void ConfigureServices(IServiceCollection services, string busName)
    {
        ...
        services.AddHealthChecks().Add(new HealthCheckRegistration(
            $"runax:{busName}",
            sp => new SqsHealthCheck(sp.GetRequiredKeyedService<IMessagingTransport>(busName)),
            failureStatus: null,
            tags: null));
    }
